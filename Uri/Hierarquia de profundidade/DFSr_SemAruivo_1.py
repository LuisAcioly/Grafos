...
def DFS_Start():
    for i in range(nTestes):
        #Toda vez que trocamos de teste a hierarquia e o tempo atual devem ser resetados
        global t_atual
        t_atual = 0
        global nEspaço
        nEspaço = 2
...
#funcao que percorre todo o grafo enquanto ouver vertice branco
def DFS(i):
    for j in range(nVertices[i]):
        #Quando um vertice é chamado aqui significa que um vertice foi fechado 
        # anteriormente(ou q se iniciou a DFS) na DFS, portando a hierarquia é resetada
        global nEspaço
        nEspaço = 2
...
def DFS_Detec(i, j):
    ...
    for u in vizinhos:
        ...
        if(cor[i][u] == 'B'):
            ...
            cor[i][u] = 'C'
            desc[i][u] = t_atual
            #Chamada da função que irá escrever a hierarquia
            pathR(j, u, visitado)
            ...
            # A hierarquia continuar no msm nivel ou retornar para anterior
            visitado = True
            pathR(j, u, visitado)
...
#funcao definida para adicionar os espacos respectivos para a hieraquia
def pathR(j, u, visitado):
    ...
    #Com base no do nEspaço calculado durante a DFS, podemos ajustar a hierarquia
